...
def main(msg: func.ServiceBusMessage):
    logging.info(
        f'station_message_queue_receiver ran at {utils.get_utc_timestamp()}')
    ...
    try:
        ...
        if action.action_type == actions.ActionType.STATIONS_ADD:
            ...
            action = pydantic.parse_raw_as(actions.AddStationsAction,
                                           message)
            cimis_response = StationService.get_stations_from_cimis(
                action.payload.station_ids)
            logging.info(
                f'Received CIMIS reponse at {utils.get_utc_timestamp()}')
            station_schema = StationService.to_station_schema(cimis_response)
            StationService.update_or_add_stations(station_schema)
            logging.info(
                f'Succesfully added station(s) at {utils.get_utc_timestamp()}')
            ...
            StationService.update_or_add_stations(station_schema)
            logging.info(
                f'Succesfully updated station(s) at {utils.get_utc_timestamp()}')
